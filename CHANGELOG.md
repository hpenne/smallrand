...
## [1.1.0](https://github.com/hpenne/smallrand/compare/v1.0.1...v1.0.2) - 2026-05-12

### Added

- Implemented ValueFromRng for signed integers.
- Refactored SecureEntropy to support a custom failure handler.

### Fixed

- Updated getrandom
- Updated doc
- DefaultRng replaces SecureEntropy as the default for StdRng
- Corrected RepetitionCountTest threshold from using > to >=.
  The test (used by SecureEntropy) now allows one less consecutive identical bytes. It is no longer recommended to use SecureEntropy in fuzzing, as it is now likely that it will eventually generate a false positive.
- Replaced std imports with core and clean up public API
